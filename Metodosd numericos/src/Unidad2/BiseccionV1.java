...
lista.add(new Intervalo(-2,-0.5));
        ...
        for (Intervalo intervalo : lista) {
            ...
            for (;;) {
                double xn = (x2 - x1) / 2.0;
                xn = x1 + xn;
                System.out.print(iteracion + "\tx1=" + x1 + "\ty1=" + fun.evaluar(x1) + "\tx2=" + x2 + "\ty2=" + fun.evaluar(x2) + "\txn=" + xn);
                if ((fun.evaluar(x1) * fun.evaluar(xn)) < 0) {
                    x2 = xn;
